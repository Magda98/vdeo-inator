...
describe('VideoDetails', () => {
  ...
  test('should render iframe', async () => {
    const dataService = createMock(DataService);
    dataService.getPopularVideos = jest.fn(() => of(videos));

    const { container } = await render(VideoDetailsComponent, {
      declarations: [VideoDetailsComponent, VideoComponent, NumberFormatPipe],
      componentProviders: [
        {
          provide: ActivatedRoute,
          useValue: {
            params: of({ id: 'xBmGxqgPbZE' })
          }
        },
        {
          provide: DataService,
